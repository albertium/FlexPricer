...
def main():
    ...
    # gamma_fn = jx.vmap(jx.grad(jx.grad(price_bs, 4), 4), in_axes=(None, 0, None, None, None, None, None))
    gamma_fn = jx.vmap(jx.grad(price_bs, 0), in_axes=(0, None, None, None, None, None, None))
    prices, (deltas, vegas) = jacob_fn(s, strikes, r, q, sig, t, innovations)
    gammas = gamma_fn(np.linspace(99.9, 100.1, 20), k, r, q, sig, t, innovations)
    print(f'It takes {time.time() - start:.2f}s')

    fig = make_subplots(3, 2)
    # fig = go.Figure()
    for idx, data in enumerate([prices, deltas, gammas]):
